    ...
    FixedVecDeque<T, STATE_COUNT>
{
    ...
    fn add(&mut self, input: T) {
        // This could be better if we used "eq_ignoring_start" (see below)
        for i in 0..self.length {
            if self.inner[self.get_idx_for_idx(i)] == input {
                return;
            }
        }
...
#[derive(Debug, Clone)]
pub struct StatesRingRcFixedUniqueRing<
...
impl<
        ...
        const STEP_COUNT: usize,
        const STATE_COUNT: usize,
    > StatesRingRcFixedUniqueRing<T, STEP_COUNT, STATE_COUNT>
{
    #[inline]
...
impl<
        ...
        const STEP_COUNT: usize,
        const STATE_COUNT: usize,
    > StatesRc<T> for StatesRingRcFixedUniqueRing<T, STEP_COUNT, STATE_COUNT>
{
    fn new() -> Self {
        Self {
            start: 0,
...
impl<
        ...
        const STEP_COUNT: usize,
        const STATE_COUNT: usize,
    > Default for StatesRingRcFixedUniqueRing<T, STEP_COUNT, STATE_COUNT>
{
    fn default() -> Self {
        Self::new()
    }
...
mod tests {
    ...
    #[test]
    fn test_idx() {
        let mut test: StatesRingRcFixedUniqueRing<usize, 10, 10> =
            StatesRingRcFixedUniqueRing::new();
    ...
    #[should_panic]
    fn test_idx_panic() {
        let test: StatesRingRcFixedUniqueRing<usize, 10, 10> = StatesRingRcFixedUniqueRing::new();
