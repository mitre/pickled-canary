...
public class LookupDataExpressionSolver {
	...
	private static long getInstructionBytes(TokenField tf, MemBuffer input, int sp) throws MemoryAccessException {
		...
		size = tf.getByteEnd() - tf.getByteStart() + 1;
		tmpsize = size;
		while (tmpsize >= 4) {
		...
		if (tmpsize > 0) {
			tmp = getBytes(input, sp + bs, tmpsize);
			res = res << (8 * tmpsize);
			res |= (tmp & 0xffffffffL);
		}
		if (!tf.isBigEndian())
	...
	public static long computeExpression(PatternExpression expression, MemBuffer input, int sp, int len) {
		if (expression instanceof BinaryExpression) {
			long leftval = computeExpression(((BinaryExpression) expression).getLeft(), input, sp, len);
			long rightval = computeExpression(((BinaryExpression) expression).getRight(), input, sp, len);
			...
			long unary = computeExpression(((UnaryExpression) expression).getUnary(), input, sp, len);
			...
			res >>= tf.getShift();
			if (tf.hasSignbit())
				res = TokenField.signExtend(res, tf.getBitEnd() - tf.getBitStart());
			else
				res = TokenField.zeroExtend(res, tf.getBitEnd() - tf.getBitStart());
			return res;
			...
			return input.getAddress().add(sp + len).getUnsignedOffset();
		} else {
